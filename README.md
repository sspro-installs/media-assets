...
> ⚠️ **Important:** Folder names and subfolder names **must be all lowercase** — GitHub URLs are case-sensitive.

> Example: use `nexo`, not `Nexo` or `NEXO`.

1. Go to the parent folder where you want the new folder created.  
   Example: `vendors/`
2. Click **“Add file” → “Create new file”**.
3. In the filename box, type the new folder path (in lowercase) followed by `/.gitkeep`.  
   Example:
   ```
...
- ⚙️ **Always use lowercase names** for folders and subfolders (e.g., `vendors/nexo/` not `vendors/Nexo/`).
...
- After uploads, wait about **1–2 minutes** for GitHub Pages to rebuild before checking the gallery.
...
| Folder name uses uppercase letters | GitHub Pages is case-sensitive | Rename folder and files to lowercase |
